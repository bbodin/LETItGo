...
#define VERBOSE_LETITGO(m) VERBOSE_CUSTOM_DEBUG("LET", m)
...
FindLongestPath(PartialConstraintGraph PKG) {
		...
		if (dist.count(src)) {
			for (Constraint c : PKG.getOutputs(src)) {
				...
				if (dist.count(dest) == 0 || dist[dest] < dist[src] + c.getWeight()) {
					...
					VERBOSE_LETITGO(" Update " << dest << " by " << src);
					VERBOSE_LETITGO(" prev =  " << prev);
				}
			}
		} else {
	...
	for (Execution e : ordered_execution) {
		...
		VERBOSE_LETITGO(e << " distance is " << dist[e]);
	}

	VERBOSE_LETITGO(prev);

	std::vector<Execution> L;
	Execution e = Execution(-1, 1);
	while (prev.count(e) == 1) {
		VERBOSE_LETITGO("Longest to " << e << " comes from " << prev.at(e));
		L.push_back(e);
		e = prev.at(e);
	}
	L.push_back(e);
...
AgeLatencyResult ComputeAgeLatency(const LETModel &model) {
	...
	while (NeedsToContinue) {
		...
		VERBOSE_LETITGO("FindLongestPath(PKG) = " << FLP);
		...
		for (Execution e : P) {
			...
			if ((K[tid] % N[tid]) != 0) {
				...
				VERBOSE_LETITGO("NiP|Ki = " << N[tid] << "|" << K[tid]
																<< " needs to continue ..");
			} else {
				VERBOSE_LETITGO("NiP|Ki = " << N[tid] << "|" << K[tid]);
			}
		...
		for (Execution e : P) {
			...
			VERBOSE_LETITGO("NiP|Ki = " << N[tid] << "|" << K[tid] << " will be "
					<< std::lcm(K[tid], N[tid]));
		}
	}

	VERBOSE_LETITGO("Final K = " << K);
