...
PartialConstraintGraph generate_partial_constraint_graph (const LETModel& model , const PeriodicityVector& K) {
	...
	for (Dependency d : model.dependencies()) {
		...
		VERBOSE_INFO("  " << "gcdeT= " << gcdeT);
		VERBOSE_INFO("  " << "gcdeK= " << gcdeK);
		VERBOSE_INFO("  " << "Me= " << Me);
		...
		for (auto ai = 1 ; ai <= Ki ; ai++) {
			for (auto aj  = 1 ; aj <= Kj ; aj++) {
				...
				VERBOSE_INFO("  " << "from " << ai << " to " << aj);

				INTEGER_TIME_UNIT alphae_ai_aj =  (Ti * ai - Tj * aj) / gcdeT ;
				INTEGER_TIME_UNIT pi_min = std::ceil( (- Me + gcdeT -  alphae_ai_aj * gcdeT ) / gcdeK ) ;
				INTEGER_TIME_UNIT pi_max = std::floor( ( - Me + Ti -  alphae_ai_aj * gcdeT ) / gcdeK ) ;


				VERBOSE_INFO("   " << "alphae_ai_aj= " << "(" << Ti << "*" << ai << "-" << Tj << "*" << aj<< ") / " << gcdeT << " = " << alphae_ai_aj);
				VERBOSE_INFO("   " << "pi_min= " << pi_min);
				VERBOSE_INFO("   " << "pi_max= " << pi_max);

				if (pi_min <= pi_max) {
					// From Theorem 6 (ECRTS2020)
					INTEGER_TIME_UNIT Lmax = rj -ri + Ti -Tj - (pi_min * gcdeK +  alphae_ai_aj * gcdeT );
					VERBOSE_INFO("   " << "Lmax= " << Lmax);
...
std::pair<std::vector<Execution> , INTEGER_TIME_UNIT>  FindLongestPath(PartialConstraintGraph PKG){
		...
		if (dist.count(src)) {
			for (Constraint c : PKG.getOutputs(src)) {
				...
				if (dist.count(dest) == 0 || dist[dest] < dist[src] + c.getWeight()) {
					...
					if (not result.second) { result.first->second = src; }
	...
	return std::pair<std::vector<Execution> , INTEGER_TIME_UNIT>(L,dist[Execution (-1,1)]);
}
	...
	while (NeedsToContinue) {
		...
		for (Execution e : P) {
			...
			if ((K[tid] % N[tid]) != 0) {
				...
				VERBOSE_INFO("NiP|Ki = " << N[tid] << "|" << K[tid] << " needs to continue ..");
			} else {
				VERBOSE_INFO("NiP|Ki = " << N[tid] << "|" << K[tid] );
			}
		}

		if (!NeedsToContinue) continue;

		for (Execution e : P) {
			if (e.first == -1) continue;
			auto tid = e.getTaskId();
			K[tid] = std::lcm(K[tid], N[tid]);
			VERBOSE_INFO("NiP|Ki = " << N[tid] << "|" << K[tid] << " will be " << std::lcm(K[tid], N[tid]));
		}
	}
